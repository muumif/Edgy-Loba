...
async function UIDToIGN(UID, platform, guildID, discordID) {
	...
	return await axios.get(encodeURI(URI))
		.then(function(response) {
			logger.info("UID API: Succesfully returned data for user: " + response.data.global.name + "/" + UID, { command: "module: UIDToIGN", guildID: guildID, discordID: discordID });
			...
			case 400:
				...
				logger.error(new Error(error), { command: "module: UIDToIGN", guildID: guildID, discordID: discordID });
